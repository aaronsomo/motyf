...
export function PhoneInput<Type extends { [k: string]: string | undefined }>({
  label,
  fields,
  setFields,
  propertyKey,
  ...
  const SSN_FORMAT = ['\\d', '\\d', '\\d', '\\-', '\\d', '\\d', '\\d', '\\-', '\\d', '\\d', '\\d', '\\d', '$'];
  ...
  const setPhone = (e: any) => {
    ...
    if (input.match(regex)) {
      if ((input.length === 3 && previous.length === 2) || (input.length === 7 && previous.length === 6)) {
